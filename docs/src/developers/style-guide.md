...
!!! info
    The style guide is always a work in progress, and not all ExpressRecipes code
    follows the rules. When modifying ExpressRecipes, please fix the style violations
...
ExpressRecipes uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/ExpressRecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the ExpressRecipes directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to ExpressRecipes have
