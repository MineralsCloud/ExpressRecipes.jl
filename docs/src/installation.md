...
[ExpressRecipes](https://github.com/MineralsCloud/ExpressRecipes.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("ExpressRecipes")
   ```

3. Run

   ```julia-repl
   julia> using ExpressRecipes
   ```
...
version of ExpressRecipes, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/ExpressRecipes.jl"
```

in the second step above.

## Update ExpressRecipes
...
our [GitHub repository](https://github.com/MineralsCloud/ExpressRecipes.jl)
for new releases.
Once we release a new version, you can update ExpressRecipes by typing

```@repl
using Pkg
Pkg.update("ExpressRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall ExpressRecipes
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("ExpressRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall ExpressRecipes.
